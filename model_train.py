...
from models.networks import MLP, BayesianPNN
...
class ModelTrain:
    def __init__(self, config) -> None:
        self.config = config
        self.pe = jax_utils.PositionalEncoding_jax(self.config.pe_level)
        self.dp = utils.DataProcessor()
        ...
        if config.model_type == 'nf':
            self.dist_narrow = distrax.MultivariateNormalDiag(jnp.zeros(1), jnp.ones(1)/20)
            ...
            self.model = NeuralSplineFlow(n_dim=config.n_dim, n_context=config.n_context, 
                            ...
                            n_transforms=config.layer_num, activation="gelu", n_bins=8)
        ...
        print(config.model_type, 'num of params:', sum(x.size for x in jax.tree_leaves(params)))
    ...
    def train(self, state, data_out, data_in, max_epoch=300, loss_threshold=0.0001):
        ...
        losses = []
        for ind in range(max_epoch):
            
            batchsize = np.minimum(self.config.batchsize, data_in.shape[0])
            train_perms = jax_utils.generate_perms(None, data_in.shape[0], batchsize)
            losses_batch = []
